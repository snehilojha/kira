...
def _format_task_state_summary(state: dict) -> str:
    """Return one compact line for a task state."""
...
def _format_task_state_detail(state: dict) -> str:
    """Return detailed text for one task state."""
    task_id = _task_state_id(state)
    request = state.get("task_request") or {}
    lines = [
        f"Task: {task_id}",
...
def _extract_check_events(state: dict) -> list[dict]:
    """Return persisted test-command tool result events."""
...
def _task_state_id(state: dict) -> str:
    """Extract a task id from a task-state payload."""
...
async def _run_complex_task_with_progress(reply, task_request) -> brain.BrainResult:
...
async def _await_brain_approval(reply, request: approval.ApprovalRequest) -> bool:
    """Send a Telegram approval prompt and wait for the user's decision."""
    ...
    try:
        ...
        await reply(
            f"Approval timed out for `{request.tool_name}`. "
            "I treated it as denied.",
            parse_mode="Markdown",
        )
        return False
...
def _build_brain_approval_text(request: approval.ApprovalRequest) -> str:
    """Format a user-facing approval prompt for a complex task action."""
...
async def _parse_monitor_job(user_message: str) -> dict | None:
    """Use the fast LLM to extract monitor job fields from natural language.

    Returns a dict with keys: name, subject, condition, poll_interval_seconds,
    success_action, failure_action (optional), expiry_at (optional).
    Returns None if the model could not parse a valid job spec.
    """
    system_prompt = (
    ...
    try:
        ...
        logger.warning("_parse_monitor_job: API call failed: %s", exc)
        return None
    ...
    try:
        ...
        if match:
            try:
                ...
                logger.warning("_parse_monitor_job: unparseable response: %r", raw)
                return None
        else:
            logger.warning("_parse_monitor_job: no JSON in response: %r", raw)
            return None

    required = ("name", "subject", "condition", "poll_interval_seconds", "success_action")
    if not all(k in parsed for k in required):
        logger.warning("_parse_monitor_job: missing required fields: %s", parsed)
        return None

    return parsed


async def _create_monitor_job_from_message(user_message: str, reply) -> bool:
    """Parse, create, and confirm a monitor job from natural language.

    Sends the user a confirmation or error message via reply().
    Returns True if a job was created, False otherwise.
    """
    spec = await _parse_monitor_job(user_message)
    if spec is None:
    ...
    try:
        ...
        logger.error("_create_monitor_job_from_message: create failed: %s", exc)
...
async def handle_ask(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """``/ask <text>`` — route natural language into the V1 ask flow."""
    ...
    try:
        ...
        if decision.route == "monitor":
            created = await _create_monitor_job_from_message(
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # --- Step 3: Route transcript and translate only simple requests ---
    try:
        ...
        if decision.route == "complex":
            ...
            audio = await _safe_synthesise(result.summary[:2000])
            if audio:
                await _send_voice(msg, audio)
            return
        ...
        if decision.route == "monitor":
            ...
            if created:
                response_text = "Monitor job created. I'll notify you when the condition is met."
            else:
                response_text = _build_route_stub_text(decision)
            audio = await _safe_synthesise(response_text)
            if audio:
...
async def handle_ask_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("brain_yes|") or data.startswith("brain_no|"):
        ...
        await query.edit_message_text(
            "Approved complex-task action." if approved else "Denied complex-task action."
        )
        return
    ...
    if command == "/run":
        ...
        if not args:
            ...
            return
        await _ask_exec_close_apps(reply, args)
...
async def _ask_exec_close_apps(reply, args: list[str]) -> None:
    """Close applications from an /ask callback."""
    result = app_control.close_apps(args)
